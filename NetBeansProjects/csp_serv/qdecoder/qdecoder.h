...
#include <time.h>
     ...
     * qcgireq.c
     */
    extern qentry_t *qcgireq_setoption(qentry_t *request, bool filemode, const char *basepath, int clearold, char **recon);
    extern qentry_t *qcgireq_parse(qentry_t *request, Q_CGI_T method, char **recon);
    extern char *qcgireq_getquery(Q_CGI_T method);

    /*
     * qcgires.c
     */
    extern bool qcgires_setcookie(qentry_t *request, const char *name, const char *value, int expire, const char *path, const char *domain, bool secure);
    extern bool qcgires_removecookie(qentry_t *request, const char *name, const char *path, const char *domain, bool secure);
    ...
    extern int qcgires_download(qentry_t *request, const char *filepath, const char *mimetype);
    extern void qcgires_error(char *format, ...);
    ...
    /* public functions */
    extern qentry_t *qentry_init(qentry_t *entry, char **recon);
    extern qentry_t *qEntry(char **recon);

    /* qentry container */
    struct qentry_s {
        // add by james 20161205
        char **_recon_;
        /* public functions */
