...
//
qentry_t *qentry_init(qentry_t *entry, char **recon) {
...
static bool _putstrf(qentry_t *entry, bool replace, const char *name, const char *format, ...) {
    char str[1024];
    STATIC_VSPRINTF(str, 1024, format);
    //
...
static char *_getstrf(qentry_t *entry, bool newmem, const char *namefmt, ...) {
    char name[1024];
    STATIC_VSPRINTF(name, 1024, namefmt);
    //
...
static bool _save(qentry_t *entry, const char *filepath) {
    ...
    char encval[2048];
    for (obj = entry->first; obj; obj = obj->next) {
        _q_urlencode(encval, obj->data, obj->size);
        fprintf(fd, "%s=%s\n", obj->name, encval);
    }
    fclose(fd);
    //
