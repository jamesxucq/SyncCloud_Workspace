...
static qentry_t *_parse_query(qentry_t *request, const char *query, char equalchar, char sepchar, int *count);
 ...
 * @endcode
 */
qentry_t *qcgireq_setoption(qentry_t *request, bool filemode, const char *basepath, int clearold, char **recon) {
    // initialize entry structure
    if (request == NULL) {
        request = qEntry(recon);
 ...
 */
qentry_t *qcgireq_parse(qentry_t *request, Q_CGI_T method, char **recon) {
    // initialize entry structure
    if (request == NULL) {
        request = qEntry(recon);
        if (request == NULL) return NULL;
...
static qentry_t *_parse_query(qentry_t *request, const char *query, char equalchar, char sepchar, int *count) {
    ...
    while (newquery && *newquery) {
        _q_makeword_s(value, newquery, sepchar);
        _q_strtrim(_q_makeword_s(name, value, equalchar));
        _q_urldecode(name);
        _q_urldecode(value);
