...
static void ngx_http_dav_ext_start_xml_elt(void *user_data, const XML_Char *name, const XML_Char **atts) {
...
static void ngx_http_dav_ext_output(ngx_http_request_t *r, ngx_chain_t **ll, ngx_int_t flags, u_char *data, ngx_uint_t len) {
    ngx_chain_t *cl;
    ngx_buf_t *b;
//
    if (!len) {
...
static ngx_int_t ngx_http_dav_ext_send_propfind_atts(ngx_http_request_t *r, char *path, ngx_str_t *uri, ngx_chain_t **ll, ngx_uint_t props) {
    struct stat st;
    struct tm stm;
    u_char buf[256];
    ngx_str_t name;
    //
    if (stat(path, &st)) {
        ngx_log_error(NGX_LOG_ALERT, r->connection->log, ngx_errno, "dav_ext stat failed on '%s'", path);
    ...
    if (props & NGX_HTTP_DAV_EXT_PROP_creationdate) {
        ...
        NGX_HTTP_DAV_EXT_OUTCB(buf, strftime((char*) buf, sizeof (buf),
                "<D:creationdate>""%Y-%m-%dT%TZ""</D:creationdate>\n", &stm));
    }
    ...
    if (props & NGX_HTTP_DAV_EXT_PROP_getcontentlength) {
        NGX_HTTP_DAV_EXT_OUTCB(buf, ngx_snprintf(buf, sizeof (buf), "<D:getcontentlength>""%O"
                "</D:getcontentlength>\n", st.st_size) - buf);
    }
    ...
    if (props & NGX_HTTP_DAV_EXT_PROP_getlastmodified) {
        ...
        NGX_HTTP_DAV_EXT_OUTCB(buf, strftime((char*) buf, sizeof (buf),
                "<D:getlastmodified>""%a, %d %b %Y %T GMT""</D:getlastmodified>\n",&stm));
    }
    ...
    if (props & NGX_HTTP_DAV_EXT_PROP_resourcetype) {
        if (S_ISDIR(st.st_mode)) {
            NGX_HTTP_DAV_EXT_OUTL("<D:resourcetype>""<D:collection/>""</D:resourcetype>\n");
        } else {
...
static ngx_int_t ngx_http_dav_ext_send_propfind_item(ngx_http_request_t *r, char *path, ngx_str_t *uri) {
    ...
    NGX_HTTP_DAV_EXT_OUTCB(vbuf, ngx_snprintf(vbuf, sizeof (vbuf), "%d.%d ", r->http_major, r->http_minor) - vbuf);
...
static void ngx_http_dav_ext_make_child(ngx_pool_t *pool, ngx_str_t *parent, u_char *child, size_t chlen, ngx_str_t *path) {
    u_char *s;
    //
    path->data = ngx_palloc(pool, parent->len + 2 + chlen);
    s = path->data;
    s = ngx_cpymem(s, parent->data, parent->len);
    if (parent->len > 0 && s[-1] != '/') *s++ = '/';
...
static ngx_int_t ngx_http_dav_ext_send_propfind(ngx_http_request_t *r) {
    ...
    if (ngx_http_variable_unknown_header(&vv, &depth_name,
            ...
            && vv.valid) {
        if (vv.len == sizeof ("infinity") - 1 && !ngx_strncasecmp(vv.data, (u_char*) "infinity", vv.len)) {
    ...
    if (p == NULL || !path.len) {
        ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "dav_ext error mapping uri to path");
        return NGX_ERROR;
    }
    path.len = p - path.data;
    *p = 0;
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http propfind path: \"%V\"", &path);
    NGX_HTTP_DAV_EXT_OUTL("<?xml version=\"1.0\" encoding=\"utf-8\" ?>\n""<D:multistatus xmlns:D=\"DAV:\">\n");
    ngx_http_dav_ext_flush(r, ll);
    /*
    ruri.data = ngx_palloc(r->pool, r->uri.len + 2 * ngx_escape_uri(NULL, r->uri.data, r->uri.len, NGX_ESCAPE_URI));
    if (ruri.data == NULL) {
        return NGX_ERROR;
    }
    ruri.len = (u_char *) ngx_escape_uri(ruri.data, r->uri.data, r->uri.len, NGX_ESCAPE_URI) - ruri.data;
     */
    ruri = r->unparsed_uri;
    ...
    if (depth) {
        ...
        if ((dir = opendir((char*) path.data))) {
            while ((de = readdir(dir))) {
                ...
                ngx_http_dav_ext_make_child(r->pool, &path, (u_char*) de->d_name, len, &spath);
                /* escape uri component */
                uc = ngx_palloc(r->pool, len + 2 * ngx_escape_uri(NULL, (u_char *) de->d_name, len, NGX_ESCAPE_URI_COMPONENT));
                if (uc == NULL) {
                    return NGX_ERROR;
                }
                uc_len = (u_char*) ngx_escape_uri(uc, (u_char *) de->d_name, len, NGX_ESCAPE_URI_COMPONENT) - uc;
...
static void ngx_http_dav_ext_propfind_handler(ngx_http_request_t *r) {
    ...
    XML_SetElementHandler(parser, ngx_http_dav_ext_start_xml_elt, ngx_http_dav_ext_end_xml_elt);
    for (; c != NULL && c->buf != NULL && !c->buf->last_buf; c = c->next) {
        b = c ->buf;
        if (!XML_Parse(parser, (const char*) b->pos, b->last - b->pos, b->last_buf)) {
            ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "dav_ext propfind XML error");
            status = NGX_ERROR;
            break;
        }
    }
...
static ngx_int_t ngx_http_dav_ext_handler(ngx_http_request_t *r) {
    ...
    switch (r->method) {
        case NGX_HTTP_PROPFIND:
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "dav_ext propfind");
            rc = ngx_http_read_client_request_body(r, ngx_http_dav_ext_propfind_handler);
            if (rc >= NGX_HTTP_SPECIAL_RESPONSE) {
                return rc;
            }
            return NGX_DONE;
        case NGX_HTTP_OPTIONS:
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "dav_ext options");
...
static char *ngx_http_dav_ext_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child) {
    ...
    ngx_conf_merge_bitmask_value(conf->methods, prev->methods, (NGX_CONF_BITMASK_SET | NGX_HTTP_DAV_EXT_OFF));
    return NGX_CONF_OK;
}
